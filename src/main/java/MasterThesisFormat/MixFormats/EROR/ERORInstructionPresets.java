...
public class ERORInstructionPresets {
    ...
    private static final byte REG_SALT_PRF = 0x18;
    ...
    public static byte[] createInstructions(byte[] nextHop, byte forwardPayloadLength, byte backwardCipherLength, byte backwardMacLength, byte[] skeSalt, byte[] prfSalt) throws IOException {
        ...
        instr.write(Instruction.load(skeSalt, REG_SALT_SKE));
        instr.write(Instruction.concate(REG_SHARED_SECRET, REG_SALT_SKE, REG_SKE_MATERIAL));
        ...
        instr.write(Instruction.verify(REG_BACKWARD_MAC, REG_FORWARD_MAC_COMPUTED));
        ...
        instr.write(Instruction.load(prfSalt, REG_SALT_PRF));
        instr.write(Instruction.concate(REG_SHARED_SECRET, REG_SALT_PRF, REG_PRF_MATERIAL));
